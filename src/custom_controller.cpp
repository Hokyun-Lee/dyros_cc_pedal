...
{
    ControlVal_.setZero();
    joystick_command = dc_.nh.subscribe("/tocabi/joystickcommand", 100, &CustomController::JoyCommandCallback, this);
    pedal_command = dc_.nh.subscribe("/tocabi/pedalcommand", 100, &CustomController::PedalCommandCallback, this);
}

void CustomController::JoyCommandCallback(const dyros_joystick::WalkingCommandConstPtr &msg){
  int test;
}
...
void CustomController::PedalCommandCallback(const dyros_pedal::WalkingCommandConstPtr &msg){
  ...
  joystick_input(1) = msg->z;
  joystick_input(2) = msg->theta;
  
  // getPedal(msg->step_length_x, msg->z, msg->theta);
}

// void CustomController::getPedal(double step_length_x, double z, double theta){
//   joystick_input_(0) = step_length_x;
//   joystick_input_(1) = z;
...
{
  joystick_input_(0) = joystick_input(0);
  joystick_input_(1) = joystick_input(1);
  joystick_input_(2) = joystick_input(2);
  std::cout << "step_length : " << joystick_input(0) << std::endl;
  std::cout << "trigger(z) : " << joystick_input(1) << std::endl;
  std::cout << "theta : " << joystick_input(2) << std::endl << std::endl;
    if (tc.mode == 10)
    {
        if(initial_flag == 0)
        {   
...
void CustomController::calculateFootStepTotal_MJ()
{
  ...
  del_size = 1;
  number_of_foot_step = 2 + 2*initial_total_step_number*del_size + middle_total_step_number*del_size + 2*final_total_step_number*del_size;
  ...
  if(initial_total_step_number != 0 || abs(initial_residual_angle) >= 0.0001) // 첫번째 회전
  {
  for (int i =0 ; i < 2*initial_total_step_number; i++)
  {
  temp *= -1;
  foot_step_(index,0) = temp*(0.1025 + step_width)*sin((i+1)/2*initial_drot);
  foot_step_(index,1) = -temp*(0.1025 + step_width)*cos((i+1)/2*initial_drot);
  foot_step_(index,5) = (i+1)/2*initial_drot;
  foot_step_(index,6) = 0.5 + 0.5*temp;
  index++;
  }
  ...
  if(final_total_step_number != 0 || abs(final_residual_angle) >= 0.0001)
  {
  for(int i = 0 ; i < 2*final_total_step_number; i++)
  {
  temp3 *= -1;

  foot_step_(index,0) = final_position_x + temp3*(0.1025 + step_width)*sin((i+1)/2*final_drot + initial_rot);
  foot_step_(index,1) = final_position_y - temp3*(0.1025 + step_width)*cos((i+1)/2*final_drot + initial_rot);
  foot_step_(index,5) = (i+1)/2*final_drot + initial_rot;
  foot_step_(index,6) = 0.5 + 0.5*temp3;
  index++;
  }
...
{ 
  ...
  if(current_step_num_ != 0)
  { 
    SC_com(0) = DyrosMath::cubic(walking_tick_mj, t_start_, t_start_ + t_total_, sc_err(0), sc_err(0), 0.0, 0.0);
    SC_com(1) = DyrosMath::cubic(walking_tick_mj, t_start_, t_start_ + t_total_, sc_err(1), sc_err(1), 0.0, 0.0);
  }
